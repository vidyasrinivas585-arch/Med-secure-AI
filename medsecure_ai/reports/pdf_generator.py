...
def _risk_colour(risk: str):
    mapping = {"Low": BRAND_GREEN, "Medium": BRAND_AMBER, "High": BRAND_RED}
...
def generate_pdf_report(data: dict, image_path: str | None = None) -> str | None:
    """
    ...
    try:
        ...
        # ── Header ──────────────────────────────────────────────────────────
        content.append(Paragraph("MedSecure AI", title_style))
        content.append(Paragraph("Medicine Authenticity Analysis Report", subtitle_style))
        content.append(Paragraph(
            f"Generated: {datetime.utcnow().strftime('%d %B %Y, %H:%M UTC')}",
            subtitle_style,
        ))
        content.append(HRFlowable(width="100%", thickness=2, color=BRAND_BLUE))
        content.append(Spacer(1, 0.4 * cm))

        # ── Medicine Image ───────────────────────────────────────────────────
        if image_path and os.path.exists(image_path):
            try:
                img = RLImage(image_path, width=6 * cm, height=6 * cm)
                img.hAlign = "CENTER"
                content.append(img)
        ...
        pred_table_data = [[
            Paragraph(
                f'<font color="white"><b>PREDICTION: {pred.upper()}</b></font>',
                ParagraphStyle("pred", fontSize=14, alignment=TA_CENTER),
            )
        ]]
        pred_table = Table(pred_table_data, colWidths=["100%"])
        pred_table.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, -1), pred_colour),
            ("PADDING",    (0, 0), (-1, -1), 10),
            ("ROUNDEDCORNERS", [8]),
        ]))
        ...
        score_data = [
            [
                Paragraph("<b>Authenticity Score</b>", body_style),
                Paragraph(f"<b>{score}%</b>", body_style),
                Paragraph("<b>Risk Level</b>", body_style),
                Paragraph(f'<font color="{risk_col.hexval()}"><b>{risk}</b></font>', body_style),
            ]
        ]
        score_table = Table(score_data, colWidths=["28%", "22%", "25%", "25%"])
        ...
        content.append(Paragraph("Medicine Information", section_style))
        info_fields = [
            ("Medicine Name",     data.get("medicine_name", "N/A")),
            ("Manufacturer",      data.get("manufacturer", "N/A")),
            ("Batch Number",      data.get("batch_number", "N/A")),
            ("Manufacturing Date", data.get("manufacturing_date", "N/A")),
            ("Expiry Date",       data.get("expiry_date", "N/A")),
            ("Expiry Status",     data.get("expiry_status", "N/A")),
            ("Days Remaining",    str(data.get("days_remaining", 0))),
        ]
        for label, value in info_fields:
            ...
            t = Table(row_data, colWidths=["40%", "60%"])
        ...
        scores_data = [
            ["Metric", "Score"],
            ["AI Confidence",       f"{data.get('ai_confidence', 0):.1f}%"],
            ["OCR Confidence",      f"{data.get('ocr_confidence', 0):.1f}%"],
            ["Packaging Score",     f"{data.get('packaging_score', 0):.1f}%"],
            ["Authenticity Score",  f"{score:.1f}%"],
        ]
        scores_table = Table(scores_data, colWidths=["60%", "40%"])
        ...
        content.append(Spacer(1, 0.3 * cm))
        content.append(Paragraph(f"<i>Report Language: {lang_display}</i>", subtitle_style))

        # ── Footer ───────────────────────────────────────────────────────
        content.append(Spacer(1, 0.5 * cm))
        content.append(HRFlowable(width="100%", thickness=1, color=colors.lightgrey))
        content.append(Paragraph(
            "⚠️  This report is generated by an AI system for screening purposes only. "
            "Always consult a licensed pharmacist or healthcare professional before making "
            "decisions based on this report.",
            ParagraphStyle("disclaimer", fontSize=8, textColor=colors.gray, alignment=TA_CENTER),
        ))
